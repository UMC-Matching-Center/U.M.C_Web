...
// 해당 Star 커스텀마이징
const EvaluateStarSVG = styled.svg`
  margin-right: 0.4rem;
`;

//차있는 Star
const EvaluateStarFilled = () => (
  <EvaluateStarSVG
...
export default function EvaluateStar({ list, editOn, setDataList }) {
  ...
  const handleStar = (index, event) => {
    if (event && event.clientX) {
      ...
      const starWidth = 12; // EvaluateStarSVG의 너비를 12로 가정
  ...
  const evaluateStarHead = Math.floor(list.rate); //4.5점일 때, 4점을 뜻함.
  const evaluateStarTail = list.rate % 1; //4.5점일 때, 0.5점을 뜻함
  ...
  const editStars = Array.from({ length: 5 }, (_, index) => {
    ...
    return (
      ...
      >
        {
          adjustedIndex <= evaluateStarHead ? (
            <EvaluateStarFilled /> // 다 채운 star
          ) : adjustedIndex === evaluateStarHead + 1 &&
            evaluateStarTail === 0.5 ? (
            <EvaluateStarHalfFilled /> // 반만 채운 star, isHoveringOut이 true일 때 같이 작동
          ) : (
            <EvaluateStarNotFilled />
          ) // 안 채워진 star
        }
      </StyledSpan>
    );
  ...
  const stars = Array.from({ length: 5 }, (_, index) => {
    if (index + 1 <= evaluateStarHead) {
      return <EvaluateStarFilled key={index} />;
    } else if (index + 1 == evaluateStarHead + 1 && evaluateStarTail == 0.5) {
      return <EvaluateStarHalfFilled key={index} />;
    } else {
      return <EvaluateStarNotFilled key={index} />;
    }
  });

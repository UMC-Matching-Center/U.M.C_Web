...
const CustomHeader4 = () => {
  ...
  const handleHeader2Click = (e) => {
    ...
    // H4 효과 (1. 드래그 선택된 값 앞에 #### , 2. 드래그 선택된 값이 없으면 커서 위치에 #### text 입력)
    const newText =
    ...
    // 커서위치 이동(비동기)
    const header4Start = postRef.current.selectionStart + 5;
    const header4End = postRef.current.selectionEnd + (selectedText ? 5 : 9);

    postRef.current.focus();
    setTimeout(() => {
      postRef.current.setSelectionRange(header4Start, header4End);
    }, 0);
  };
  return (
    <button onClick={handleHeader2Click}>
      <IconH4 size={24} color="#393556" stroke={1} />
    </button>
  );
};

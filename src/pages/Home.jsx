...
const Home = () => {
    ...
    const changeViewCount=(count)=>{setMoreCount(count)}
    const handleShowMoreCount=()=>{setCardCount((prev)=>prev+moreCount)}
    /*필터 메뉴*/
    ...
    const optionList = [
        {num:3,content:'3개씩 보기'}, {num:15,content:'15개씩 보기'}, {num:30,content:'30개씩 보기'},{num:60,content:'60개씩 보기'}
    ];
            ...
            <div className="app__main-contents">
                <div className='app__main-filter'>
                    <div className='app__main-filterWrap'>
                        ...
                        <SelectBox className='app__main-filter-content' onClick={()=>setIsShowOptions((prev)=>!prev)}>
                            <Label>{currentValue}</Label>
                            <SelectOptions show={isShowOptions}>
                                {optionList.map((option,i)=>
                                    <Option onClick={()=>handleOnChangeOption(option.num,option.content)} key={i}>{option.content}</Option>
                                )}
                            </SelectOptions>
                        </SelectBox> 
                    </div>
                ...
                <div className='app__main-projects'>
                    {projectsDummy.slice(0,cardCount).map((project,index)=>{
                        return <Card project={project} key={index} onClick={handleProjectClick}/>
                    })}
                </div>
            </div>
            <div>
